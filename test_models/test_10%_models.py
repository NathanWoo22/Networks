...
def main():
    ...
    for i, filename in enumerate(noise_numbers_percentage):
        ...
        full_path = "epos_10_percentage/" + str(noise_numbers_percentage[i])

        # Check if it's a subdirectory
        if os.path.isdir(full_path):
            print(f'Directory: {full_path}')    
            MF, correlation_coefficient = gen.generate_plots(full_path, full_path, "epos_noise_percentage/Epos_10.0.npz", "Mu = 10%, " + "Sigma = 10%")
            MFs.append(MF)
            file.write(str(MF) + ", ")
    file.close()
